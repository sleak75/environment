...
"sjl: tweak to highlist when in an indented block inside rst file:
syntax match plantumlPreProc /\%(^\s*@startuml\|^\s*@enduml\)\|!\%(define|definelong|else|enddefinelong|endif|if|ifdef|ifndef|include|pragma|undef\)\s*.*/ contains=plantumlDir
...
" Note
"syntax region plantumlNote start=/\%(^\s*note\)/ end=/^\%(\s*erd\s*note$\)/ contained contains=plantumlString,plantumlSpecialString
...
"syntax region plantumlNoteMultiLine start=/\%(^\s*note\)\%([^"]\+$\)/ end=/^\%(\s*end \?note$\)/ contains=plantumlSpecialString,plantumlNoteMultiLineStart
...
"syntax match plantumlNoteMultiLineStart /\%(^\s*note\)/ contained contains=plantumlKeyword,plantumlColor,plantumlString
""syntax match plantumlNoteMultiLineStart /\%(^\s*[rh]\?note\)\@<=\s\%([^:]\+$\)/ contained contains=plantumlString
...
"sjl highlight default link plantumlDir Constant
highlight default link plantumlDir PreProc
